<?php
// includes/okta_auth_flexible.php - Flexible Okta integration that accepts multiple issuer formats
class OktaAuthFlexible {
    private $config_file;
    private $config;
    ...
    public function get_authorization_url() {
        ...
        // Try org-level endpoints first, but store multiple possible issuers
        $_SESSION['okta_domain'] = $domain;
        $_SESSION['okta_possible_issuers'] = [
            "https://{$domain}",
            "https://{$domain}/oauth2/v1",
            "https://{$domain}/oauth2/default",
            $domain  // Sometimes Okta just uses domain without protocol
        ];
        ...
        $auth_url = "https://{$domain}/oauth2/v1/authorize?" . http_build_query($params);
        error_log("DEBUG: Flexible auth using org-level URL: " . $auth_url);
        return $auth_url;
    }
    ...
    public function handle_callback($code, $state) {
        ...
        // Validate and decode ID token with flexible issuer validation
        $id_token_payload = $this->validate_id_token_flexible($token_data['id_token'] ?? '');
    ...
    private function exchange_code_for_tokens($code, $code_verifier) {
        ...
        error_log("DEBUG: Flexible auth using token endpoint: " . $token_url);
        ...
        error_log("DEBUG: Token exchange successful with flexible auth");
        return $token_data;
    }
    
    private function validate_id_token_flexible($id_token) {
        if (empty($id_token)) {
        ...
        // Get possible issuers from session
        $possible_issuers = $_SESSION['okta_possible_issuers'] ?? [
            "https://{$domain}",
            "https://{$domain}/oauth2/v1", 
            "https://{$domain}/oauth2/default"
        ];
        
        error_log("DEBUG: Flexible issuer validation");
        error_log("DEBUG: Actual ID token issuer: " . $actual_issuer);
        error_log("DEBUG: Possible expected issuers: " . implode(', ', $possible_issuers));
        ...
        // Flexible issuer validation - accept any of the possible issuers
        $issuer_valid = false;
        $matched_issuer = '';
        
        if (isset($payload['iss'])) {
            foreach ($possible_issuers as $possible_issuer) {
                if ($payload['iss'] === $possible_issuer) {
                    $issuer_valid = true;
                    $matched_issuer = $possible_issuer;
                    break;
                }
            }
            
            // If exact match fails, try partial match (in case of additional path components)
            if (!$issuer_valid) {
                foreach ($possible_issuers as $possible_issuer) {
                    if (strpos($payload['iss'], $possible_issuer) === 0) {
                        $issuer_valid = true;
                        $matched_issuer = $possible_issuer;
                        error_log("DEBUG: Issuer matched with partial match: " . $matched_issuer);
                        break;
                    }
                }
            }
        ...
        if (!$issuer_valid) {
            // Store the actual issuer for debugging
            error_log("DEBUG: ISSUER MISMATCH DETAILS:");
            error_log("DEBUG: - Actual issuer: " . $actual_issuer);
            error_log("DEBUG: - Expected any of: " . implode(', ', $possible_issuers));
            error_log("DEBUG: - Domain configured: " . $domain);
            
            // Create a more helpful error message
            $possible_list = implode(', ', $possible_issuers);
            throw new Exception("ID token issuer mismatch. Expected one of: {$possible_list}, but got: {$actual_issuer}. Check debug log for details.");
        }
        
        error_log("DEBUG: Issuer validation passed with: " . $matched_issuer);
        return $payload;
    }
    ...
    private function get_user_info($access_token) {
        ...
        error_log("DEBUG: Using flexible userinfo endpoint: " . $userinfo_url);
        ...
        error_log("DEBUG: UserInfo retrieved successfully with flexible auth");
        return $user_info;
    }
    ...
    private function process_user($user_data) {
        ...
        $_SESSION['login_method'] = 'okta_sso_flexible';
        ...
        unset($_SESSION['okta_domain']);
        unset($_SESSION['okta_possible_issuers']);
        
        // Log the successful login
        $this->log_login($email, $groups, $user_role['role'], $user_data);
        
        error_log("DEBUG: User session created successfully for: " . $email . " using flexible auth");
        return true;
    }
    ...
    private function log_login($email, $groups, $role, $user_data) {
        ...
        $audit_logs[] = [
            ...
            'timestamp' => date('c'),
            'action' => 'user_login_okta_flexible',
            'entry' => 'User Login via Okta SSO (Flexible)',
            'user' => $email,
    ...
    public function test_connection() {
        ...
        if ($http_code === 400 || $http_code === 401) {
            return [
                'success' => true,
                'message' => 'Successfully connected to Okta (Flexible Org Authorization Server)',
                'auth_server' => 'Flexible Org Authorization Server',
                'endpoints' => [
                    'authorization' => "https://{$domain}/oauth2/v1/authorize",
                    'token' => "https://{$domain}/oauth2/v1/token",
                    'userinfo' => "https://{$domain}/oauth2/v1/userinfo",
                    'issuer' => "https://{$domain} (flexible)"
                ]
            ];
        } else {
            return [
